...
export default function FacultiesPage() {
  ...
  const handleEdit = (faculty: Faculty) => {
    ...
    setFormData({ nom: faculty.nom, description: faculty.description || "" });
    setIsModalOpen(true);
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm("Voulez-vous supprimer cette faculté ?")) return;
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const updated = await facultyService.updateFaculty(editingId, formData);
        setFaculties(prev => prev.map(f => f.id === editingId ? updated : f));
        ...
        <tbody>
          {faculties.map((faculty) => (
            <Tr key={faculty.id}>
              ...
              <Td>
                <ActionButtons>
                  <ActionButton onClick={() => handleEdit(faculty)}>
                    <FaEdit />
                  </ActionButton>
                  <ActionButton onClick={() => handleDelete(faculty.id)}>
                    <FaTrash />
                  </ActionButton>
                </ActionButtons>
              </Td>
      ...
      <ModalOverlay isOpen={isModalOpen} onClick={() => setIsModalOpen(false)}>
        <Modal onClick={(e) => e.stopPropagation()}>
          <Title style={{ fontSize: "1.5rem", marginBottom: "20px" }}>
            {editingId ? "Modifier" : "Ajouter"} une faculté
          </Title>
